...
```{r}
model2_fit = sem(model_RT, filt_df, missing = "ML", estimator = "MLR")
```

```{r}
summary(model2_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```

Third model with SL_AGL as predictor:
...
# dependent regression
TROG ~ b11*stroop + b12*PS_visdec + b13*PS + b14*nback + b15*DS + b16*simon + c11*SL_AGL
MENYET ~ b21*stroop + b22*PS_visdec + b23*PS + b24*nback + b25*DS + b26*simon + c21*SL_AGL
reading ~ b31*stroop + b32*PS_visdec + b33*PS + b34*nback + b35*DS + b36*simon + c31*SL_AGL
pred ~ b41*stroop + b42*PS_visdec + b43*PS + b44*nback + b45*DS + b46*simon + c41*SL_AGL
OMR ~ b51*stroop + b52*PS_visdec + b53*PS + b54*nback + b55*DS + b56*simon + c51*SL_AGL

# mediator regression
stroop ~ a11*SL_AGL
PS_visdec ~ a21*SL_AGL
PS ~ a31*SL_AGL
nback ~ a41*SL_AGL
DS ~ a51*SL_AGL
simon ~ a61*SL_AGL
...
```{r}
model3_fit = sem(model_AGL, filt_df, missing = "ML", estimator = "MLR")
```

```{r}
summary(model3_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```

Fourth model with SL_SEGM as only predictor:
...
# dependent regression
TROG ~ b11*stroop + b12*PS_visdec + b13*PS + b14*nback + b15*DS + b16*simon + c11*SL_SEGM
MENYET ~ b21*stroop + b22*PS_visdec + b23*PS + b24*nback + b25*DS + b26*simon + c21*SL_SEGM
reading ~ b31*stroop + b32*PS_visdec + b33*PS + b34*nback + b35*DS + b36*simon + c31*SL_SEGM
pred ~ b41*stroop + b42*PS_visdec + b43*PS + b44*nback + b45*DS + b46*simon + c41*SL_SEGM
OMR ~ b51*stroop + b52*PS_visdec + b53*PS + b54*nback + b55*DS + b56*simon + c51*SL_SEGM

# mediator regression
stroop ~ a11*SL_SEGM
PS_visdec ~ a21*SL_SEGM
PS ~ a31*SL_SEGM
nback ~ a41*SL_SEGM
DS ~ a51*SL_SEGM
simon ~ a61*SL_SEGM
...
```{r}
model4_fit = sem(model_SEGM, filt_df, missing = "ML", estimator = "MLR")
```

```{r}
summary(model4_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```

```{r}
reliability(model1_fit)
reliability(model2_fit)
reliability(model3_fit)
reliability(model4_fit)
```
