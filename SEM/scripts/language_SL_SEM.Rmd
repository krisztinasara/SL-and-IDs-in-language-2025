...
SL_RT =~ SEGM_AL_medRT_TRN3_RND4 + SEGM_AL_medRT_RND4_REC5
"
SL_RT_model_fit = sem(SL_RT_model, df, missing = "listwise")
implied_measurement(SL_RT_model_fit, latent = "SL_RT")
```
...
#SEGM_AL_2AFC_trigram ~~ SEGM_AL_2AFC_bigram
"
model_SL_offline_fit = sem(model_SL_offline, df, missing = "listwise")
#implied_measurement(model_SL_offline_fit, latent = "SL_offline", subset = 1:6)

residual_plots(model_SL_offline_fit, max_val = 0.01)
```
...
```{r}
model_2_fit = sem(model_2, df, missing = "listwise", estimator = "ML")
```

```{r}
model_3 =
"
nback =~ n_back_nback_2_dprime + n_back_nback_3_dprime + n_back_nback_1_dprime
#nback1 =~ n_back_nback_1_dprime
"

model_3_fit = sem(model_3, df, missing = "listwise")
```

```{r}
implied_measurement(model_2_fit, latent = "nback", subset = 1:6)
```

```{r}
visualize(model_2_fit)
```

```{r}
residual_plots(model_3_fit, max_val = .001)
#residual_plots(model_3_fit, max_val = .05)
```
