...
```{r}
filt_df = read.csv(
  "C:/Users/Kriszti/GitHub/lendulet_language_SL/SEM/dataframes/filtered_SEM_df.csv"
)
```

```{r}
# Change reliability values here!
stroop_e = (1-0.768)*var(filt_df$stroop_RT_score, na.rm = TRUE)
simon_e = (1-0.643)*var(filt_df$simon_RT_score, na.rm = TRUE)
visdec_e = (1-0.952)*var(filt_df$PROC_SPEED_vis_dec_RT_med, na.rm = TRUE)
menyet_e = (1-0.676)*var(filt_df$MENYET_mean_ACC_all, na.rm = TRUE)
trog_e = (1-0.63)*var(filt_df$trog_pragm_ACC_mean, na.rm = TRUE)
pred_e = (1-0.853)*var(filt_df$predictive_RT_score, na.rm = TRUE)
```
...
OMR ~ b51*nback + b52*DS + b53*PS + b54*stroop + b55*simon + b56*PS_visdec + c51*SL_RT + c52*SL_offline
...
```{r}
model1_fit = sem(model1, df, missing = "ML", estimator = "MLR")
```
...
2. Step 2: throw out pred -- outcome variable with very low explained variance
...
```{r}
model2_fit = sem(model2, df, missing = "ML", estimate = "MLR")
```

```{r}
summary(model2_fit, standardized = TRUE, fit.indices = TRUE)
```

```{r}
reliability(modelx_fit)
```
