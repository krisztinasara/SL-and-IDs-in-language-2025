...
implied_measurement(model_SL_offline_fit, latent = "SL_offline")
```
...
```{r}
implied_measurement(model_3_fit, latent = "nback")
```

```{r}
visualize(model_3_fit, plot = "latent", formula = nback ~ nback1)
```

```{r}
residual_plots(model_3_fit, max_val = 0.5)
```
