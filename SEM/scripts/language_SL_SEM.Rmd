...
```{r}
model_2_fit = bootstrapLavaan(model_1_fit, R = 10, type = "ordinary")
```

```{r}
summary(model_2_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```
...
```{r}
model_3_fit = sem(model_3, df,  missing = "ML", estimator = "ML")
```

```{r}
summary(model_3_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```

```{r}
model_5 =
"
SL_offline =~ SEGM_AL_2AFC_trigram + AGL_2AFC_phr + AGL_2AFC_sent + AGL_prod + SEGM_AL_2AFC_bigram
MENYET =~ MENYET_mean_ACC_all
TROG =~ trog_pragm_ACC_mean
MENYET ~ SL_offline
TROG ~ SL_offline
...
```{r}
model_5_fit = sem(model_5, df, missing = "pairwise", estimator = "ML")
```

```{r}
summary(model_5_fit, standardized = TRUE, rsquare = TRUE, fit.measures = TRUE)
```
```{r}
modificationIndices(model_5_fit)
```
