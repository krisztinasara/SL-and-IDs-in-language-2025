---
title: "EFA with indices n>100 in the adult sample"
...
1. iteration
-- excluding simon_ACC_score due to low factor loading etc. --> single score factor?

2. iteration
-- excluding simon_RT_score due to low factor loading etc. --> single score factor?

3. iteration
-- excluding PROC_SPEED_vis_dec_RT_med because it is a single score factor

Remaining question: Stroop ACC needed in the Stroop factor?
...
Remaining question: very low commonality of MENYET
```{r}
LA_EFA = df %>% 
  select(
    MENYET_mean_ACC_all,
  ...
  fa(nfactors = 1, fm ="ml", rotate = "Promax")
```
