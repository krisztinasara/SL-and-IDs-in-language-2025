...
Our initial dataset comprised 12 tasks with 45 indices (Table XX). Each
index has a minimum overlap of 100 with another index. For each index,
two criteria were considered: (i) **unidimensionality** and (ii)
**internal consistency**, indicative of its reliability. Indices failing
to meet either criterion were dropped. Indices that couldn't be
...
The *trog_gramm* index was excluded due to ceiling effects. For
*grammatical sensitivity* and *pragmatic comprehension* indices, the
...
[Insert table XX from Supplementary material 2]
...
To identify latent variables, we initially categorized the measurement
...
In the next step, exploratory factor analyses (EFAs) were conducted
...
indices were dropped due to model refinement. Thus, the final dataset
included 23 indices from 12 different tasks (Table XXY).
...
AGL_ACC_train Then, KMO test was repeated.
...
In one case (*SEGM_AL_2AFC_bigram*), the unexplained variance was high
(\>0.65), however we kept it to assign convergent reliability to the
SL_trigram variable. We further excluded the *SEGM_AL_midRT_train*
variable to solve the Heywood case present in this factor.
...
Indices with unsatisfactory fit for EFA (MSA \< 0.6) were excluded: \*
simon_ACC_score \* stroop_ACC_score Then, KMO test was repeated.
...
*PROC_SPEED_vis_dec_RT_med* index as a single-variable latent factor
retaining the other two variables as the *PS factor*.
...
We excluded the *predictive_ACC_score* index which showed unacceptable
KMO values (\< 0.5).
...
*predictive_RT_score* is also near the boundary.
...
## Fit indices for all the mediation models
