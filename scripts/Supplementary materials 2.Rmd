...
```{r}
library(ggpol)
boxplotting = function(data, variable, baseline, variable_name) {
  ggplot(data, aes(y = variable)) +
    geom_boxjitter(
      outlier.color = NA, jitter.shape = 21, jitter.color = NA, jitter.fill = "black",
      jitter.params = list(height = 0.025, width = 0.1),
      errorbar.draw = FALSE
      ) +
...
boxplotting(
  data = df,
  variable = df$perceptual_speed_visual_RT,
  baseline = -0.5,
  variable_name = "perceptual speed - visual reaction time"
)
```
