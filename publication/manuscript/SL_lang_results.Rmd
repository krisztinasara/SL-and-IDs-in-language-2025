...
Our initial dataset comprised 12 tasks with 45 indices (Table 1). Each
index has a minimum overlap of 100 cases with each other indices. For each
index, two criteria were considered: (i) **unidimensionality** and (ii)
...
In one case (*SEGM_AL_2AFC_bigram [speech segmentation - SEGM 2AFC
...
Indices with unsatisfactory fit for EFA (MSA \< 0.6) were excluded: \*
simon_ACC_score [simon - accuracy] \* stroop_ACC_score [stroop -
accuracy]
...
the BIC. So for further analyses, we used the *PROC_SPEED_vis_dec_RT_med
...
We excluded the *predictive_ACC_score [predictive sentence processing -
...
*predictive_RT_score [predictive sentence processing - RT]* is also near
the boundary.
...
-   selfpaced_target_RT_diff_GP [self-paced reading - garden path
    sentence processing]
-   selfpaced_target_RT_diff_sertes [self-paced reading - sentence
...
*Reliabilities and average variance extracted (AVE) for latent
variables*
