...
```{r include = FALSE}
library(readr)
df = read_delim("~/GitHub/lendulet_language_SL/SEM/dataframes/SEM_df_manuscript.csv", 
...
df = df %>% 
  mutate(
    processing_speed_visual_RT = -(processing_speed_visual_RT),
    processing_speed_visual_decision = -(processing_speed_visual_decision),
    processing_speed_auditory_RT = -(processing_speed_auditory_RT),
    simon_RT = -(simon_RT),
...
To identify latent variables, we initially categorised the measurement
indices into three designated domains: the *statistical learning* (SL)
domain, the *general cognitive* (COG) domain, and the *language* (LANG)
domain (see Table 1).

As a next step, exploratory factor analyses (EFAs) were conducted
within each domain, following these procedures:
...
variables to identify latent variables which later serve as predictors
...
df %>% 
  select(
    processing_speed_visual_RT,
    processing_speed_auditory_RT,
    processing_speed_visual_decision,
    digit_span_forward,
    digit_span_backward,
    nback_1_back_dprime,
    nback_2_back_dprime,
...
Indices with unsatisfactory fit for EFA (MSA \< 0.6) were excluded: \*
simon_accuracy [simon - accuracy] \* stroop_accuracy [stroop -
accuracy]
...
the general cognitive variables as well.
...
-   processing_speed_visual_RT [processing speed - visual reaction time]
-   processing_speed_auditory_RT [processing speed - auditory reaction time]
-   processing_speed_visual_decision [processing speed - visual decision time]
...
model_COG =
"
  COG_PS =~ processing_speed_visual_RT + processing_speed_visual_decision + processing_speed_auditory_RT
...
model_PS_01 =
"
  PS_01 =~ processing_speed_visual_RT + processing_speed_visual_decision
  PS_02 =~ processing_speed_auditory_RT
  COG_DS =~ digit_span_forward + digit_span_backward
...
model_PS_02 =
"
  PS_01 =~ processing_speed_visual_RT + processing_speed_auditory_RT
  PS_02 =~ processing_speed_visual_decision
  COG_DS =~ digit_span_forward + digit_span_backward
  COG_nback =~ nback_2_back_dprime + nback_3_back_dprime
  COG_stroop =~ stroop_RT 
  COG_simon =~ simon_RT
...
the BIC. So for further analyses, we used the *processing_speed_visual_decision
[processing speed - visual decision time]* index as a single-variable
latent factor retaining the other two variables as the *PS factor*.
...
We excluded the *predictive_sent_proc_accuracy [predictive sentence processing -
accuracy]* index which showed unacceptable KMO values (\< 0.5).
...
*predictive_sent_proc_RT [predictive sentence processing - RT]* is also near
the boundary.
...
identify latent variables which serve as outcome in the SEM models.
...
statistical learning variable, all the general cognitive variables, and
the language variables. We specified the models with lavaan syntax. CFA
...
visdec_e = (1-0.955)*var(df$processing_speed_visual_decision, na.rm = TRUE)
...
# cognitive
PS =~ processing_speed_visual_RT + processing_speed_auditory_RT
DS =~ digit_span_forward + digit_span_backward
nback =~ nback_2_back_dprime + nback_3_back_dprime
stroop =~ stroop_RT
simon =~ simon_RT
PS_visdec =~ processing_speed_visual_decision
...
processing_speed_visual_decision ~~ 0.043*processing_speed_visual_decision
...
# cognitive
COG =~ processing_speed_visual_RT + processing_speed_auditory_RT + digit_span_forward + digit_span_backward + nback_2_back_dprime + nback_3_back_dprime + stroop_RT + simon_RT + processing_speed_visual_decision
